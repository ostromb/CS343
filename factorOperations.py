...
def joinFactors(factors):
    ...
    for f in factors:
        uncond.update(f.unconditionedVariables())
        cond.update(f.conditionedVariables())
    for s in uncond:
        if s in cond:
            cond.remove(s)
    factor = Factor(uncond, cond, varDomainsDict)
    assignments = factor.getAllPossibleAssignmentDicts()
    for assign in assignments:
        prob = 1
        for f in factors:
            prob *= f.getProbability(assign)
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        f = Factor(uncond, cond, varDomainsDict)

        for assign in f.getAllPossibleAssignmentDicts():
            prob = 0
            for val in domain:
                prev = assign
                prev[eliminationVariable] = val
                prob += factor.getProbability(prev)

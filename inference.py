...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        allpos = DiscreteDistribution()
        for p in self.allPositions:
            allpos[p] = self.beliefs[p] * self.getObservationProb(
                observation, gameState.getPacmanPosition(), p, self.getJailPosition())
        allpos.normalize()
        self.beliefs = allpos
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        totparticles = 0
        while totparticles < self.numParticles:
            for pos in self.legalPositions:
                if totparticles < self.numParticles:
                    totparticles += 1
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.particles:
            weights[p] += self.getObservationProb(
                observation, gameState.getPacmanPosition(), p, self.getJailPosition())
        if (weights.total() == 0):
            self.initializeUniformly(gameState)
        ...
        for i in range(self.numParticles):
            particles.append(weights.sample())
    ...
    def elapseTime(self, gameState):
        ...
        particles = []
        for p in self.allPositions:
            particles[p] = self.getPositionDistribution(gameState, p)
        for particle in self.particles:
            particles.append(position[particle].sample())
        self.particles = particles
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                if ghost is None:
                    ...
                    particle[i] = jailPosition
                    particle = tuple(particle)
                else:
                    prob = prob * \
                        self.getObservationProb(
                            ghost, gameState.getPacmanPosition(), particle[i], jailPosition)
            dist[particle] += prob
        dist.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for g in range(self.numGhosts):
                dist = self.getPositionDistribution(
                    gameState, oldParticle, g, self.ghostAgents[g])
                sample = dist.sample()
                newParticle[g] = sample

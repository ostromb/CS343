...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for pair in self.mdp.getTransitionStatesAndProbs(state, action):
            r = self.mdp.getReward(state, action, pair[0])
            q += pair[1] * (r + (self.discount * self.values[pair[0]]))
        return q
...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for s in self.mdp.getStates():
            if not self.mdp.isTerminal(s):
                for a in self.mdp.getPossibleActions(s):
                    for pair in self.mdp.getTransitionStatesAndProbs(s, a):
                        nextState = pair[0]
        ...
        for i in range(self.iterations):
            ...
            for p in preds[state]:
                a = self.computeActionFromValues(p)

    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            variableDomainsDict[obsVar] = [
                BLUE_OBS_VAL, RED_OBS_VAL, NO_OBS_VAL]
...
def fillObsCPT(bayesNet, gameState):
    ...
    def getPos(house):
        pos = None
        if abs(house[0] - bottomLeftPos[0]) <= 1 and abs(house[1] - bottomLeftPos[1]) <= 1:
            pos = BOTTOM_LEFT_VAL
        elif abs(house[0] - bottomRightPos[0]) <= 1 and abs(house[1] - bottomRightPos[1]) <= 1:
            pos = BOTTOM_RIGHT_VAL
        elif abs(house[0] - topLeftPos[0]) <= 1 and abs(house[1] - topLeftPos[1]) <= 1:
            pos = TOP_LEFT_VAL
        elif abs(house[0] - topRightPos[0]) <= 1 and abs(house[1] - topRightPos[1]) <= 1:
            pos = TOP_RIGHT_VAL
        return pos

    for house in gameState.getPossibleHouses():
        for wall in gameState.getHouseWalls(house):
            varDomains = bayesNet.variableDomainsDict()
            obs = OBS_VAR_TEMPLATE % wall
            factor = bn.Factor(
                [obs], [GHOST_HOUSE_VAR, FOOD_HOUSE_VAR], varDomains)

            pos = getPos(house)

            for assign in factor.getAllPossibleAssignmentDicts():
                prob = 0
                if assign[FOOD_HOUSE_VAR] == pos:
                    if assign[obs] == RED_OBS_VAL:
                        prob = PROB_FOOD_RED
                    elif assign[obs] == BLUE_OBS_VAL:
                        prob = 1 - PROB_FOOD_RED
                    else:
                        prob = 0
                elif assign[GHOST_HOUSE_VAR] == pos:
                    if assign[obs] == RED_OBS_VAL:
                        prob = PROB_GHOST_RED
                    elif assign[obs] == BLUE_OBS_VAL:
                    ...
                    if assign[obs] == NO_OBS_VAL:
                        prob = 1
                    else:
                        prob = 0

                factor.setProbability(assign, prob)
            bayesNet.setCPT(obs, factor)
...
class BayesAgent(game.Agent):
    ...
    def getRandomAction(self, gameState):
        ...
        successors = [gameState.generatePacmanSuccessor(
            a).getPacmanPosition() for a in legal]
        ls = [(a, s) for a, s in zip(legal, successors)
              if s not in gameState.getPossibleHouses()]
        ls.sort(key=lambda p: p[1] in self.visited)
        return ls[0][0]

    def getEvidence(self, gameState):
        evidence = {}
        for ePos, eColor in list(gameState.getEvidence().items()):
    ...
    def goToBest(self, gameState):
        ...
        eliminationOrder = unknownVars + \
            [X_POS_VAR, Y_POS_VAR, GHOST_HOUSE_VAR]
        bestFoodAssignment = getMostLikelyFoodHousePosition(evidence,
                                                            self.bayesNet, eliminationOrder)

        tx, ty = dict(
            list(zip([BOTTOM_LEFT_VAL, TOP_LEFT_VAL, BOTTOM_RIGHT_VAL, TOP_RIGHT_VAL],
...
class VPIAgent(BayesAgent):
    ...
    def getExplorationProbsAndOutcomes(self, evidence):
        ...
        for nRed in range(8):
            ...
            outcomeEvidence = dict(list(zip(unknownVars, outcomeVals)))
    ...
    def enterAction(self, gameState, left=True):
        ...
        best = min(list(zip(actions, neighborStates)),
...
def cacheDistances(state):
    ...
    walls = state.getWalls().asList() + state.data.layout.redWalls.asList() + \
        state.data.layout.blueWalls.asList()

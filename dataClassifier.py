...
def enhancedFeatureExtractorDigit(datum):
    ...
    Added a feature called "cycle" - used depth-first search to figure out if there's a cycle
    ##
    """
    ...
    def dfs(w, h):
        if w < 0 or w >= DIGIT_DATUM_WIDTH:
            return False
        if h < 0 or h >= DIGIT_DATUM_HEIGHT:
            return False
        if datum.getPixel(w, h) == 2 or datum.getPixel(w, h) == 1:
            return True
        if (w, h) in checked.keys():
            return True
        checked[(w, h)] = True
        return dfs(w+1, h) and dfs(w, h+1) and dfs(w-1, h) and dfs(w, h-1)
